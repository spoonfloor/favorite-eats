...
> **Status:** **Complete (2026-05).** Browser SQL.js / local-blob persistence and
> the `js/main.js` tail are removed. Postgres via `window.dataService` is the
> only runtime data path. `recipe-lan-server/` was **kept** (separate Python
> LAN experiment; not browser sql.js).
>
> **Related:** `docs/migration-sweep.md`, `docs/supabase-architecture.md`.
...
## Phase 7 — `recipe-lan-server/` (separate product — **kept**)

Not browser sql.js. **Decision:** keep for LAN experiments; out of scope for browser cleanup.
...
- [x] Phase 1 — `js/main.js` runtime scrub + smoke
- [x] Phase 2 — `js/sqliteBlobCache.js` deleted
- [x] Phase 3 — 14 HTML script tags removed; build checked
- [x] Phase 4 — `sql.js` removed from `package.json`; lockfile updated
- [x] Phase 5 — data layer comments/contracts/fixtures
- [x] Phase 6 — tests updated; `npm test` green
- [x] Phase 7 — `recipe-lan-server/` **kept** (LAN experiment; out of browser scope)
- [x] Phase 8 — docs and cursor rules updated
- [x] Phase 9 — incidental cleanup
- [x] Phase 10 — `rg` gate + `npm test` + `test:web-build`
