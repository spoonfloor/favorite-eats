...
**Related:** list/session semantics live in `docs/catalog-plan-list-supabase.md`; broader migration context in `docs/multi-device-roadmap.md`. **Warm-client implementation plan:** `docs/agent-handoff-warm-client-performance.md` (canonical on `feature/warm-client`).
...
- **Web-only runtime:** Electron entrypoints (`electronMain.js`, `preload.js`) and npm `electron` / `electron-builder` dependencies removed. App code no longer branches on `window.electronAPI` / `isElectron`. The supported behavioral axes are **editing vs planning** (`window.plannerMode`, planner `localStorage` keys)—not delivery channel.
...
- **2026-05-21:** **Warm client Slice 1 (shipped on `feature/warm-client`)** — `catalog.get_shopping_revisions()` RPC; `js/favoriteEatsStore.js` with monotonic `applyRemote`, sessionStorage snapshot, revision probe before `load_shopping_state`, single-flight hydrate preserved, apply-block retry via existing coalesced path. Code: migration `20260530120000_get_shopping_revisions.sql`, `js/favoriteEatsStore.js`, `js/data/index.js`, `js/data/adapters/supabaseAdapter.js`, `js/main.js`, `tests/runFavoriteEatsStoreTests.js`.
- **2026-05-21:** **Warm client Slice 4 (shipped on `feature/warm-client`)** — `catalog.load_items_screen()` and `catalog.load_recipes_screen(p_plan_updated_at)` replace 8+ catalog GETs + separate plan hydrate on Items/Recipes cold load; `get_shopping_revisions` now includes `catalogUpdatedAt`; `js/favoriteEatsCatalogCache.js` (IndexedDB write-through for Items warm revisit). Code: migration `20260530140000_load_items_and_recipes_screen.sql`, `js/screens/items.js`, `js/screens/recipes.js`, adapter + `main.js`, `shopping.html`, `recipes.html`.
- **2026-05-21:** **Warm client Slice 3 (shipped on `feature/warm-client`)** — Remote shopping authority: after first remote apply, `sessionStorage` flag `favoriteEats:remote-shopping-authority:v1` stops legacy localStorage→server bridge; `maintainShoppingPlanStorageWithDb` runs only on invalidation (`runFavoriteEatsRemoteShoppingPlanRefresh` / catalog reference → plan hydrate), not on Shopping List / Items / Recipes / Stores cold paint. Code: `js/main.js`, `docs/agent-handoff-warm-client-performance.md`.
- **2026-05-21:** **Warm client Slice 2 (shipped on `feature/warm-client`)** — `catalog.load_shopping_list_screen()` RPC bundles revisions + plan + list doc + filtered recipe summaries; Shopping List cold load uses one screen RPC instead of separate hydrate + catalog refresh + recipe-summary prefetch. Plan rows still fetched separately (server-side planRows deferred). Code: migration `20260530130000_load_shopping_list_screen.sql`, `js/screens/shoppingList.js`, `js/data/adapters/supabaseAdapter.js`, `js/data/index.js`, `js/main.js`, `shoppingList.html`.
