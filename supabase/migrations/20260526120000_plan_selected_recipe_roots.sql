-- Persist plan.recipeSelectionRoots server-side so save/load round-trips do not
-- re-mirror merged counts (root + implied-from-linked-recipes) into roots, which
-- caused recipe quantities to grow by the implied amount on every page load.
--
-- Fixes the "shopping list quantity creep" bug where, e.g., Waldorf Salad's
-- quantity grew by +1 per visit because Meatball Subs' linked-recipe row pointed
-- to Waldorf with empty quantity (treated as 1). See .cursor/rules/shopping-state-known-fragility.mdc.

create table if not exists plan.selected_recipe_roots (
  document_id bigint not null references plan.documents(id) on delete cascade,
  recipe_id bigint not null,
  title text not null default '',
  quantity numeric not null,
  servings_override numeric,
  inbound_link_depth integer,
  updated_at timestamp with time zone not null default now(),
  primary key (document_id, recipe_id)
);

alter table plan.selected_recipe_roots enable row level security;

drop policy if exists plan_allow_all_selected_recipe_roots
  on plan.selected_recipe_roots;

create policy plan_allow_all_selected_recipe_roots
  on plan.selected_recipe_roots
  for all
...
grant all on plan.selected_recipe_roots to anon, authenticated;
...
begin
  ...
  if state_payload ? 'plan' then
    ...
    -- Always mirror recipeSelectionRoots if the payload included a `plan` blob,
    -- even when the roots key is absent (treat as empty so we clear stale state).
...
-- Seed plan.selected_recipe_roots from existing plan.selected_recipes.
-- Without this, the first save (after the new RPC) would clear roots before the
-- client has had a chance to populate them, which would wipe all merged entries
...
select
  ...
  sr.recipe_id,
  sr.title,
  -- Conservative seed: assume the existing merged quantity == intended root.
  -- Existing inflated values (e.g., Waldorf=26) get reset by the data-cleanup
  -- migration that follows; for everyone else this is exactly correct because
  -- they have no incoming linked-recipe edges in the current dataset.
  least(sr.quantity, 99),
  sr.servings_override
from plan.selected_recipes sr
on conflict (document_id, recipe_id) do nothing;
