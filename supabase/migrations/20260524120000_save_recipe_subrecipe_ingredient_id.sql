-- save_recipe: valid linked-subrecipe rows must still resolve ingredient_id.
-- Previously the PL/pgSQL branch skipped all ingredient lookup for is_recipe rows,
-- leaving v_ingredient_id null so inserts could fail or rows were dropped on reload.
...
begin
  ...
  loop
    ...
    if not (v_is_recipe and v_linked_recipe_id is not null and v_linked_recipe_id <> v_recipe_id) then
      ...
      -- Linked subrecipe: catalog rows still need ingredient_id; derive label from payload or linked recipe title.
      v_ingredient_name := btrim(coalesce(v_ing->>'ingredient_name', v_ing->>'recipe_text', ''));
      if v_ingredient_name = '' then
        select title into v_ingredient_name
          from recipes
         where id = v_linked_recipe_id
         limit 1;
      end if;
      v_ingredient_name := btrim(coalesce(v_ingredient_name, ''));
      if v_ingredient_name = '' then
        continue;
      end if;
    end if;
