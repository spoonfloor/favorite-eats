-- Variant-only store aisle rows should not create a base item aisle link.
  ...
  loop
    ...
    loop
      ...
      select
        ...
        or exists (
          select 1
           ...
           where lower(btrim(selected_variant.value #>> '{}')) in ('', 'default', 'base', 'any')
        )
      ...
      loop
        ...
        if lower(v_variant_name) in ('', 'default', 'base', 'any') then
          continue;
        end if;
