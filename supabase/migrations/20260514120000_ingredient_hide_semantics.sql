-- Align ingredient flags with UI: Hidden = is_hidden, Removed = is_deprecated.
-- Legacy hide_from_shopping_list is merged into is_hidden where needed, then cleared.

update catalog.ingredients i
set is_hidden = true
where coalesce(i.hide_from_shopping_list, false) = true
  and coalesce(i.is_deprecated, false) = false
  and coalesce(i.is_hidden, false) = false;

update catalog.ingredients
set hide_from_shopping_list = false;
  ...
  loop
    ...
    loop
      ...
      loop
        ...
        if v_variant_name = '' or lower(v_variant_name) = 'default' then
          continue;
        end if;
