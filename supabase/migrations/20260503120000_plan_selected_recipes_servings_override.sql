-- plan.selected_recipes.quantity = make-count (how many times the recipe is in the plan).
-- servings_override stores the web-mode servings choice for ingredient scaling (per recipe in the plan).

alter table plan.selected_recipes
  add column if not exists servings_override numeric;
...
begin
  ...
  if state_payload ? 'plan' then
    ...
    loop
      ...
      v_servings_override := nullif(
        coalesce(nullif(v_recipe->>'servingsOverride', ''), nullif(v_recipe->>'servings_override', '')),
        ''
      )::numeric;
